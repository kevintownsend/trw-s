...
void InitGraph(const char* vdata_file, floatField &mrf) {
    ...
    uint32_t size = sizeof(FieldPackage) + pixelCount *(sizeof(label_t) + sizeof(floatEdges)) + pixelCount; //TODO: calc size requirements
    // Allocate array of size width x height
    uint32_t tmp = sizeof(FieldPackage);
    cerr << "mrf size:" << sizeof(floatField) << endl;
    ...
    fieldPackage->edges = tmp;
    tmp += pixelCount * sizeof(floatEdges);
    fieldPackage->assignment = tmp;
    tmp += pixelCount;
    fieldPackage->size = size;
    assert(tmp == size);
    mrf.data = (label_t*)((uint8_t*)fieldPackage + fieldPackage->data);
    mrf.edges = (floatEdges*)((uint8_t*)fieldPackage + fieldPackage->edges);
    ...
    for(int i=0; i < mrf.width * mrf.height; i++) {
        for(int k=0; k < LABELS; k++) {
            mrf.edges[i].left[k] = 0;
            mrf.edges[i].right[k] = 0;
            mrf.edges[i].up[k] = 0;
            mrf.edges[i].down[k] = 0;
        }
    }
...
TYPE MAP(floatField mrf)
{
    ...
    for(size_t i=0; i < mrf.width * mrf.height; i++) {
        ...
        for(int j=0; j < LABELS; j++) {
            ...
            cost += mrf.edges[i].left[j];
            cost += mrf.edges[i].right[j];
            cost += mrf.edges[i].up[j];
            cost += mrf.edges[i].down[j];
            cost += mrf.data[i][j];
