...
void trws(floatField mrf){
    ...
    for(int i = 0; i < height; i++){
        for(int j = 0; j < width; j++){
            ...
            label_t right, down;
            for(int k = 0; k < 16; k++){
                float sum = mrf.data[index][k]*SCALE + mrf.array[index].left[k] + 
                    mrf.array[index].right[k] + mrf.array[index].up[k] + mrf.array[index].down[k];
                //TODO: down message
                if(sum >255)
                    sum = 255;
                float tmp = sum*FRAC - mrf.array[index].down[k];
                down[k] = tmp;
                tmp = sum*FRAC - mrf.array[index].right[k];
                right[k] = tmp;
                //TODO: right message
            }
            //smoooth
    ...
    for(int i = height-1; i >= 0; i--){
        for(int j = width-1; j >= 0; j--){
            ...
            label_t left, up;
            for(int k = 0; k < 16; k++){
                float sum = mrf.data[index][k]*SCALE + mrf.array[index].right[k] + 
                    mrf.array[index].left[k] + mrf.array[index].down[k] + mrf.array[index].up[k];
                //TODO: up message
                if(sum >255)
                    sum = 255;
                float tmp = sum*FRAC - mrf.array[index].up[k]*FRAC;
                up[k] = tmp;
