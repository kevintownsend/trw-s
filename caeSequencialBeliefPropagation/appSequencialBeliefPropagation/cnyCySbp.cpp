...
extern "C" void bpsAll();

extern "C" void custom0All();
extern "C" void loadAeg0All();
extern "C" void load_dataAll();
extern "C" void belief_downAll();
extern "C" void belief_upAll();
extern "C" void belief_down_upAll();
extern "C" void store_downAll();
extern "C" void store_upAll();
using namespace std;
...
int main(int argc, char* argv[]) {
  ...
  if (cny_cp_interleave() == CNY_MI_3131) {
    ...
    FieldPackage* fpCont = splitGraph(resizedMrf, hSplits, vSplits, fields);
...
//print 8x8 block
    ...
    cout << "moving " << endl;
    cny_cp_memcpy(cnyFp, fpCont, hSplits * vSplits * fpSize);
    cout << "starting bps" << endl;
    void* blank = cny_cp_malloc(fpSize);
    //stealTardis();
    ...
    markTime();
    if(false)
    for(int h = 0; h < hSplits * vSplits; h+=4){
        //copcall_fmt(sig, loadAeg0All, "AAAA", blank, (uint64_t)(cnyAddress), blank, blank);
        copcall_fmt(sig, loadAeg0All, "AAAA", (cnyAddress), cnyAddress + fpSize, cnyAddress + 2 * fpSize, cnyAddress + 3 * fpSize);
        copcall_fmt(sig, load_dataAll, "");
        /*
        for(int i = 0; i < runs - 1; i++){
            copcall_fmt(sig, belief_downAll, "");
            copcall_fmt(sig, belief_upAll, "");
        }*/
        //markTime();
        copcall_fmt(sig, belief_down_upAll, "A", (uint64_t)(runs-1));
        copcall_fmt(sig, belief_downAll, "");
        copcall_fmt(sig, store_downAll, "");
        copcall_fmt(sig, belief_upAll, "");
        copcall_fmt(sig, store_upAll, "");
        //copcall_fmt(sig, bps, "AA", (uint64_t)(cnyFp + h * fpSize), (uint64_t)runs);
        /*
        for(int i=0; i < 40; i++) {
    ...
    returnTardis();
    //returnTardis();
    cny_cp_memcpy(fpCont, cnyFp, hSplits * vSplits * fpSize);
